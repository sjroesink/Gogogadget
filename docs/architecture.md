...
## Windows-iconen en gebruik (0.1.8)

De native icon-adapter gebruikt IShellItemImageFactory voor bestaande IDs uit de lokale AppsFolder-index. Shell-aanroepen draaien buiten de UI-thread, met maximaal twee actieve extracties en begrensde wachttijden. Een vastgelopen extractie behoudt zijn semaphore-permit om onbegrensde worker-groei te voorkomen. COM-, bitmap- en DC-resources worden via RAII vrijgegeven. De adapter retourneert een PNG-data-URL; de frontend vraagt alleen zichtbare/bijna zichtbare resultaten op. Beide caches zijn begrensd tot 256 items en leven alleen tijdens de sessie. Ontbrekende iconen houden de bestaande SVG-fallback.

Gebruik wordt na een geslaagde command-run vanuit de resultatenlijst opgeslagen in settings.json, onder een sleutel van itemsoort en ID. Dit meet launcher-openingen, geen systeemwijde procesactiviteit. Maximaal 2.000 tellers worden bewaard. Zoekfiltering gebeurt vóór sortering en resultaatbegrenzing, zodat veelgebruikte apps ook buiten de eerste 40 indexitems omhoog kunnen komen. Sortering en tellers blijven behouden bij herstart; bestaande instellingen migreren zonder historische telling.
