...
De eerdere generieke fout combineerde readonly-, focus- en selectiecontroles. De launcher verbergt nu zijn eigen venster vóór terugschrijven. De native worker wacht begrensd op werkelijke foreground- en veldfocus; SetForegroundWindow kan asynchroon activeren. UIA SetFocus wordt alleen aangevraagd als het veld zijn focus niet zelf terugkrijgt. Bewerken wordt gecontroleerd op de actuele range, met ValuePattern als fallback wanneer TextPattern geen readonly-attribuut publiceert. Inhoud en beide positiegrenzen blijven vereist. Fouten zijn uitgesplitst en blijven zichtbaar in de chat.

Een nieuwe optionele RichEdit-integratietest heeft de volledige capture/focuswissel/restore/Ctrl+V-route eenmaal succesvol uitgevoerd, inclusief exacte Unicode-inhoud en behoud van omliggende tekst. Een uitgebreide herhaling om ook gelijke tekst op een andere positie te testen strandde bij de testopstelling: Windows weigerde foreground-activatie van het synthetische bronvenster. Die aanvullende interactieve weigeringstest is daarom nog niet bevestigd. De gewone test voor vertraagde focus, verstreken deadline en foutpropagatie draait zonder desktopinteractie. Uitvoeren van de interactieve test: `cargo test --manifest-path src-tauri/Cargo.toml native_focus_restore_and_paste_roundtrip -- --ignored --nocapture`. Deze test opent uitsluitend twee tijdelijke testvensters en wijzigt het klembord.

Windows beschrijft de asynchrone activering in [SetForegroundWindow and asynchronous activation](https://devblogs.microsoft.com/oldnewthing/20161118-00/?p=94745).
## App-iconen en gebruik 0.1.8

- 31 TypeScript-tests geslaagd, één optionele live-test overgeslagen. Nieuwe tests controleren gescheiden itemtellers, instellingenmigratie, ongeldige tellingen en sortering vóór resultaatbegrenzing met behoud van zoekfiltering.
- 9 gewone native tests geslaagd, 5 optionele tests overgeslagen. De optionele installed_notepad_icon-test is afzonderlijk geslaagd met het echte geïnstalleerde Windows Kladblok-icoon.
- Browserpreview: Settings geopend vanuit de resultatenlijst, Most used gekozen en herladen. Teller en sortering bleven behouden, Settings stond bovenaan. Lay-out visueel gecontroleerd. Native app-iconen zijn via de Shell-extractietest gecontroleerd, niet in deze browserpreview.
- TypeScript-build, Rust Clippy met -D warnings en Windows-releasebuild met NSIS-installer geslaagd. Initieel JavaScript 46,13 kB / 16,17 kB gzip; Markdown blijft lazy geladen. Geen nieuwe startup- of geheugencijfers gemeten.
